...
void ThreadPool::Add(Task t) {
    std::unique_lock<mutex> lk(mutexPool);
    if (shutdown) {
        return;
    }
    //添加任务
...
void ThreadPool::Add(ThreadPool::Task::callback f, void* a) {
    std::unique_lock<mutex> lk(mutexPool);
    if (shutdown) {
        return;
    }
...
void ThreadPool::worker(void* arg) {
    ...
    while (true) {
        ...
        std::unique_lock<mutex> lk(pool->mutexPool);
        // 当前任务队列是否为空
